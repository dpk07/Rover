defmodule RoverTest do
  ...
  test "Moving a North-facing rover to the left." do
    {:ok, rover} = Rover.init(%Rover{direction: @north, x: -5, y: 4}) |> Rover.move(@left, 2)
  ...
  test "Moving a South-facing rover to the left." do
    {:ok, rover} = Rover.init(%Rover{direction: @south, x: 3, y: 7}) |> Rover.move(@left, 10)
  ...
  test "Moving an East-facing rover to the left." do
    {:ok, rover} = Rover.init(%Rover{direction: @east, x: -10, y: -20}) |> Rover.move(@left, 100)
    assert rover.direction == @north
    assert rover.x == -10
    assert rover.y == 80
  end
  ...
  test "Moving a West-facing rover to the left." do
    {:ok, rover} = Rover.init(%Rover{direction: @west, x: 0, y: 0}) |> Rover.move(@left, 2)
  ...
  test "Moving a North-facing rover to the right." do
    {:ok, rover} =
      Rover.init(%Rover{direction: @north, x: -205, y: 400}) |> Rover.move(@right, 129)
  ...
  test "Moving a South-facing rover to the right." do
    {:ok, rover} =
      Rover.init(%Rover{direction: @south, x: 1002, y: 729}) |> Rover.move(@right, 200)
  ...
  test "Moving an East-facing rover to the right." do
    {:ok, rover} = Rover.init(%Rover{direction: @east, x: -5, y: 4}) |> Rover.move(@right, 5)
    assert rover.direction == @south
    assert rover.x == -5
    assert rover.y == -1
  end
  ...
  test "Moving a West-facing rover to the right." do
    {:ok, rover} = Rover.init(%Rover{direction: @west, x: -100, y: 247}) |> Rover.move(@right, 22)
    assert rover.direction == @north
    assert rover.x == -100
    assert rover.y == 269
  end
